...
Point *global_points1, *global_points2;
int global_num_pairs= 0;
double global_pair_distance = INT_MAX;
...
int main(int argc, char *argv[])
{
    ...
    Point *local_points;
    global_points1 = (Point *)malloc(num_points * sizeof(Point));
    global_points2 = (Point *)malloc(num_points * sizeof(Point));
    for(int i = 0; i < num_points; i++)
    {
        global_points1[i].num_dimensions = num_dimensions;
        global_points1[i].coordinates = (int *)malloc(num_dimensions * sizeof(int));
        global_points2[i].num_dimensions = num_dimensions;
        global_points2[i].coordinates = (int *)malloc(num_dimensions * sizeof(int));
    }
    ...
    if(comm_size <= 2)
    {
        ...
        printf("SUPER FINAL GLOBAL DMIN: %f\n", sequential_closestpair_recursive(all_points, num_points));
        }
        ...
        for (int i = 0; i < j - 1; i++){
            for (int k = i + 1; k < j && (local_strip_points[k].coordinates[1] - local_strip_points[i].coordinates[1]) < global_dmin; k++){
                ...
                if(dd < global_pair_distance){
                    ...
                    global_points1[0] = local_strip_points[i];
                    global_points2[0] = local_strip_points[k];
                    global_num_pairs = 1;

                }
                else if(dd == global_pair_distance){
                    global_points1[global_num_pairs] = local_strip_points[i];
                    global_points2[global_num_pairs] = local_strip_points[k];
                    global_num_pairs++;
                }
                if (dd < min){
                    min = dd;
    ...
    printf("[RANK %d] global global_num_pairs: %d Super_final_dmin: %f global: %f\n", rank_process, global_num_pairs, super_final_dmin, global_pair_distance);
    if(super_final_dmin == global_pair_distance){
        for(int i = 0; i < global_num_pairs; i++){
            if(i ==0 || differPoint(global_points1[i],global_points1[i-1]) || differPoint(global_points2[i],global_points2[i-1])){
            printPoint(global_points1[i]);
            printPoint(global_points2[i]);
            printf("\n");
            }
        }
    }
