...
int main(int argc, char *argv[])
{
    ...
    if(comm_size == 1){
        ...
        Point *all_points = (Point *)malloc(num_points * sizeof(Point));
    ...
    if (rank_process == MASTER_PROCESS)
    {
        ...
        local_points = (Point *)malloc(num_points * sizeof(Point));
        for (int point = 0; point < num_points; point++){
            local_points[point].num_dimensions = num_dimensions;
            local_points[point].coordinates = (int *)malloc(num_dimensions * sizeof(int));
            for (int dimension = 0; dimension < num_dimensions; dimension++)
                local_points[point].coordinates[dimension] = all_points[point].coordinates[dimension];
        }
    }
    }
    else
